...
Board::Board(std::size_t grid_width, std::size_t grid_height)
    : width(grid_width), height(grid_height) {
  std::unique_lock<std::mutex> lck(mtx_);
  std::vector<std::vector<CellContent>> temp(
      width,
      std::vector<Board::CellContent>(height, Board::CellContent::Empty));
