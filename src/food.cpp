...
Food::Food(size_t width, size_t height)
    : engine(dev()), random_food(1, 3), random_w(0, static_cast<int>(width)),
...
void Food::SetBoard(Board *board_) { board = board_; }
...
void Food::CheckExpired() {
  ...
  millisecs_t duration(
      std::chrono::duration_cast<millisecs_t>(end - lastTimePlaced));
...
void Food::Place() {
  ...
  while (true) {
    ...
    if (board->GetCellValue(x, y) == Board::CellContent::Empty) {
      ...
      board->SetCellValue(pre_x, pre_y, Board::CellContent::Empty);
